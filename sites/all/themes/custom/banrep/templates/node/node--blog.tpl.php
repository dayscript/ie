...
<article class="<?php print $classes; ?> clearfix node-<?php print $node->nid; ?>"<?php print $attributes; ?>>
  ...
  <?php
    ...
    hide($content['links']);
    print '<span class="date"><strong>Fecha de publicación:</strong> ' . format_date($node->created, 'custom', 'd/m/Y') . '</span>';
    print drupal_render($content['field_imagen']);
    print '<div class="content-body">' . $node->body['und'][0]['value'] . '</div>';
    if(isset($node->field_tags['und'])){
      $tags = '<div class="tags"><h4>Etiquetas</h4><ul>';
      foreach ($node->field_tags['und'] as $key => $tag) {
        $tags .= '<li>' . l($tag['entity']->name, 'entradas-blog/tags/' . $tag['tid']) . '</li>';
      }
      $tags .= '</ul></div>';
      print $tags;
    }
